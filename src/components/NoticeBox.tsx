...
const noticeBand = cva(
    "flex items-center gap-2 px-4 py-2 font-sans text-meta font-bold uppercase tracking-caps",
    {
        variants: {
            tone: {
...
const noticeBody = cva("grid gap-4 px-5 py-4", {
    variants: {
        tone: {
...
export function NoticeBox({
    ...
    return (
        <aside
            className={cn("ui-notice", `ui-notice--${tone ?? "danger"}`, noticeBox({ tone }), className)}
            role="note"
            {...props}
        >
